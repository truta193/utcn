...
toList (Cursor left element right) =
    List.reverse left ++ (element :: right)
...
forward (Cursor left element right) =
    case right of
        [] ->
            Nothing

        next :: tail ->
            Just (Cursor (element :: left) next tail)
...
back (Cursor left element right) =
    case left of
        [] ->
            Nothing

        first :: tail ->
            Just (Cursor tail first (element :: right))
